...
<section>
    ...
    <div class="card-body">
        <div class="row mb-3">
            <div class="col-6">
                <p class="judul-menu"><i></i>BAT Category</p>
            </div>
            <div class="col-6">
                <div class="d-flex justify-content-end align-items-center w-100">
                    <div class="input-group style-search mx-4">
                        <input id="searchBat" type="text" class="form-control" placeholder="Search" aria-describedby="basic-addon2">
            ...
            <table class="table">
                ...
                <tbody id="dataAllBat">
                    {{-- @for($i = 1; $i <= 10; $i++)
                        <tr>
                            <td>{{$i}}</td>
                            <td>Salah Input Program / Offset / Lifetime</td>
                            <td>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Add BAT</h5>
            </div>
            <div class="modal-body">
                <div class="card-auto">
                    <div class="row">
                        <label for="" class="textarea-label col-4">Description</label>
                        <div class="col-8">
                            <textarea name="bat_desc"  cols="30" rows="10" class="form-control"></textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="modal_edit" tabindex="-1" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Edit BAT</h5>
            ...
            <div class="modal-body">
                <div class="card-auto">
                    <div class="row">
                        <label for="" class="textarea-label col-4">Description</label>
                        <div class="col-8">
                            <textarea name="bat_desc" id="bat_desc" cols="30" rows="10" class="form-control"></textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        fetch("{{ route('get_all_bat') }}", { // Laravel Blade syntax to generate the route URL
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            var table = document.getElementById('dataAllBat');
...
<script>
    ...
    document.getElementById('save_add').addEventListener('click', function() {
        ...
        var batDesc    = document.querySelector('textarea[name="bat_desc"]').value;
        ...
        .then(data => {
            if(data.status == 'success'){
                swal.fire({
                    ...
                    modal.hide();
                    // Clear the form
                    document.querySelector('textarea[name="bat_desc"]').value = '';
                    // Append the new data to the table
                    var table = document.getElementById('dataAllBat');
                    table.innerHTML = data.arr
                });            
...
<script>
    $(document).on("click", "[data-name='edit']", function (e) {
        ...
        .then(data => {
            var bat_desc   = data.arr[0].bat_desc;
            // select2 option set id plant
            document.getElementById('bat_desc').value  = bat_desc;
        })
        .catch((error) => {
            // Handle errors here
            console.error('Error:', error);
    ...
    document.getElementById('save_edit').addEventListener('click', function() {
        ...
        var batDesc    = document.querySelector('textarea[id="bat_desc"]').value;
        ...
        // Fetch API to send the data
        fetch("{{ route('edit_bat') }}", { // Laravel Blade syntax to generate the route URL
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // CSRF token for Laravel
...
<script>
    $(document).on("click", "[data-name='delete']", function (e) {
        var id       = $(this).attr('data-item');
        var bat_desc = $(this).attr('data-arr');
        
        Swal.fire({
            title: 'Anda yakin ?',
            text: 'Delete Bat Description : ' +bat_desc + ' ?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                // Fetch API to get the data
                fetch("{{ route('delete_bat') }}", { // Laravel Blade syntax to generate the route URL
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // CSRF token for Laravel
...
<script>
    $(document).ready(function(){
        $("#searchBat").on("keyup", function() {
            var value = $(this).val().toLowerCase();
            $("#dataAllBat tr").filter(function() {
