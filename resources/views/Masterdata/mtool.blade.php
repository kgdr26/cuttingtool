...
<section>
    ...
    <div class="card-body">
        <div class="row mb-3">
            <div class="col-6">
                <p class="judul-menu"><i></i>Tool Type</p>
            </div>
            <div class="col-6">
                <div class="d-flex justify-content-end align-items-center w-100">
                    <div class="input-group style-search mx-4">
                        <input id="searchTool" type="text" class="form-control" placeholder="Search" aria-describedby="basic-addon2">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="card-auto">
                    <div class="row mb-3">
                        <label for="" class="text-label col-4">Tool ID</label>
                        <div class="col-8">
                            <input type="text" class="form-control" name="id_tool">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-8">
                            <input type="text" class="form-control" name="tool_type">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-body">
                <div class="card-auto">
                    <div class="row mb-3">
                        <label for="" class="text-label col-4">Tool ID</label>
                        <div class="col-8">
                            <input type="text" class="form-control" name="id_tool" id="id_tool">
                        </div>
                    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        fetch("{{ route('get_all_tool') }}", { // Laravel Blade syntax to generate the route URL
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            var table = document.getElementById('dataAllTool');
...
<script>
    ...
    document.getElementById('save_add').addEventListener('click', function() {
        ...
        var idTool      = document.querySelector('input[name="id_tool"]').value;
        var toolType    = document.querySelector('input[name="tool_type"]').value;

        var data = {
            id_tool: idTool,
            tool_type: toolType        };
        // Fetch API to send the data
        fetch("{{ route('add_tool') }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // CSRF token for Laravel
        ...
        .then(data => {
            if(data.status == 'success'){
                swal.fire({
                    ...
                    modal.hide();
                    // Clear the form
                    document.querySelector('input[name="id_tool"]').value = '';
                    document.querySelector('input[name="tool_type"]').value = '';
                    // Append the new data to the table
                    var table = document.getElementById('dataAllTool');
                    table.innerHTML = data.arr
                });            
...
<script>
    $(document).on("click", "[data-name='edit']", function (e) {
        ...
        .then(data => {
            var id_tool    = data.arr[0].id_tool;
            var tool_type  = data.arr[0].tool_type;
            // select2 option set id plant
            document.getElementById('id_tool').value   = id_tool;
            document.getElementById('tool_type').value = tool_type;
    ...
    document.getElementById('save_edit').addEventListener('click', function() {
        ...
        var idTool     = document.querySelector('input[id="id_tool"]').value;
        var toolType   = document.querySelector('input[id="tool_type"]').value;
        ...
        // Fetch API to send the data
        fetch("{{ route('edit_tool') }}", { // Laravel Blade syntax to generate the route URL
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // CSRF token for Laravel
...
<script>
    $(document).on("click", "[data-name='delete']", function (e) {
        var id        = $(this).attr('data-item');
        var id_tool = $(this).attr('data-arr');

        Swal.fire({
            title: 'Anda yakin?',
            text: 'Delete ID TOOL : ' + id_tool + ' ?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                // Fetch API to get the data
                fetch("{{ route('delete_tool') }}", { // Laravel Blade syntax to generate the route URL
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') // CSRF token for Laravel
                    },
                    body: JSON.stringify({id: id, id_tool: id_tool})
                })
                .then(response => {
                    if (!response.ok) {
                        throw new Error('Network response was not ok: ' + response.statusText);
...
<script>
    $(document).ready(function(){
        $("#searchTool").on("keyup", function() {
            ...
            $("#dataAllTool tr").filter(function() {
